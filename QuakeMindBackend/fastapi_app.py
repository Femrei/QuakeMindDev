...
def road_damage_assembly(
    ...
    # bbox_from_center gives a square, so corner results can be up to ~radiusKm*sqrt(2)
    # away. Filter to an actual circle, sort by distance, and cap the count so the map
    # shows the nearest handful of spots instead of every park in the whole city.
    radius_m = radiusKm * 1000
    nearby_records = []
    for rec in records:
        lat = rec.get("display_lat", rec.get("lat"))
        lon = rec.get("display_lon", rec.get("lon"))
        distance_m = _haversine_m(latitude, longitude, lat, lon)
        if distance_m <= radius_m:
            enriched = dict(rec)
            enriched["distanceM"] = round(distance_m, 1)
            nearby_records.append(enriched)

    MAX_TOTAL_RECORDS = 20
    official_nearby = sorted(
        (r for r in nearby_records if r.get("priority") == 0), key=lambda r: r["distanceM"]
    )
    candidate_nearby = sorted(
        (r for r in nearby_records if r.get("priority") != 0), key=lambda r: r["distanceM"]
    )
    remaining_slots = max(0, MAX_TOTAL_RECORDS - len(official_nearby))
    records = sorted(official_nearby + candidate_nearby[:remaining_slots], key=lambda r: r["distanceM"])
